...
def find_ffmpeg():
    ...
    possible_paths = [
        ...
        os.path.join(os.path.dirname(sys.executable), 'ffmpeg') if getattr(sys, 'frozen', False) else None,
...
def _normalize_proxy_entry(entry: str):
    ...
    if clean.count(":") == 3 and "@" not in clean and "://" not in clean:
        try:
...
PROXY_SOURCES = [
    ...
    {"name": "mmpx12-proxy-list", "url": "https://raw.githubusercontent.com/mmpx12/proxy-list/master/http.txt", "parser": _parse_proxy_lines},
    {"name": "roosterkid-proxy-list", "url": "https://raw.githubusercontent.com/roosterkid/openproxylist/main/HTTPS_RAW.txt", "parser": _parse_proxy_lines},
    {"name": "proxyscrape-free-list", "url": "https://api.proxyscrape.com/v4/free-proxy-list/get?request=displayproxies&protocol=http&timeout=10000&country=all&ssl=all&anonymity=all&skip=0&limit=2000", "parser": _parse_proxy_lines},
    {"name": "proxifly-http-list", "url": "https://cdn.jsdelivr.net/gh/proxifly/free-proxy-list@main/proxies/protocols/http/data.txt", "parser": _parse_proxy_lines},
    {"name": "hideip-http-list", "url": "https://github.com/zloi-user/hideip.me/raw/refs/heads/master/http.txt", "parser": _parse_proxy_lines},
    ...
    {"name": "sunoochi-proxy-list", "url": "https://raw.githubusercontent.com/sunoochi/Proxy-List/main/http.txt", "parser": _parse_proxy_lines},
    {"name": "userr3x-proxy-list", "url": "https://raw.githubusercontent.com/UserR3X/proxy-list/main/online/http.txt", "parser": _parse_proxy_lines},
    {"name": "clarketm-proxy-list", "url": "https://raw.githubusercontent.com/clarketm/proxy-list/master/proxy-list-raw.txt", "parser": _parse_proxy_lines},
    {"name": "aslisk-proxy-list", "url": "https://raw.githubusercontent.com/aslisk/proxyhttps/main/https.txt", "parser": _parse_proxy_lines},
    {"name": "rdavydov-proxy-list", "url": "https://raw.githubusercontent.com/rdavydov/proxy-list/main/proxies/http.txt", "parser": _parse_proxy_lines},
    {"name": "mertguvencli-proxy-list", "url": "https://raw.githubusercontent.com/mertguvencli/http-proxy-list/main/proxy-list/data.txt", "parser": _parse_proxy_lines},
    {"name": "proxylist-geonode-api", "url": "https://proxylist.geonode.com/api/proxy-list?limit=500&page=1&sort_by=lastChecked&sort_type=desc&protocols=http%2Chttps", "parser": _parse_geonode_payload, "is_api": True},
    {"name": "speedx-socks5-list", "url": "https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/socks5.txt", "parser": _build_scheme_parser("socks5")},
    {"name": "speedx-socks4-list", "url": "https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/socks4.txt", "parser": _build_scheme_parser("socks4")},
...
class ProxyManager:
        ...
        for allow_skipped in (False, True):
            for source in _iter_sources(allow_skipped):
                ...
                try:
                    ...
                    if added:
                        self._log(f"Lấy được {added} proxy từ {name}")
                except Exception as exc:
                    self._log(f"Không thể lấy proxy từ {name}: {exc}")
    ...
    def load_from_file(self, file_path: str):
        ...
        try:
            ...
            raise IOError(f"Không đọc được file proxy: {exc}") from exc
...
""")
    ...
    def _can_start_generation(self):
        ...
        if hasattr(self, 'disable_proxy_btn') and self.disable_proxy_btn.isChecked():
            return True
    ...
    def _on_proxy_file_loaded(self, path: str, count: int, error: str | None):
        ...
        if self.proxy_loader_thread:
            ...
            self.proxy_loader_thread = None
        if error:
            self.proxy_file_status.setText("Lỗi khi nạp proxy.")
            QMessageBox.warning(self, "Lỗi nạp proxy", error)
            self._append_log(f"❌ Lỗi nạp proxy: {error}")
            return

        if count > 0:
            self.proxy_file_status.setText(f"Đã nạp {count} proxy sống.")
            self._append_log(f"✅ Đã nạp {count} proxy từ file {os.path.basename(path)}.")
            if self._can_start_generation():
                self._try_start_pending_prompts()
        else:
            self.proxy_file_status.setText("Không có proxy sống.")
            self._append_log("⚠️ File proxy không có proxy còn sống.")
    ...
    def task_finished(self, prompt, result):
        ...
        if result["status"] == "SUCCESSFUL":
            ...
            self.table.item(row, 2).setText("⏹️ Đã dừng")  # Cột 2: Trạng thái
            self.table.setCellWidget(row, 3, QLabel("—"))  # Cột 3: Xem
            self._set_completion_time(row, datetime.now().strftime("%H:%M:%S"))
            # Timer sẽ tự động launch worker tiếp theo sau 2s (thuật toán cũ)

        else:
            reason = result.get("reason", "Lỗi không rõ.")
            self.table.item(row, 2).setText("❌ Thất bại")  # Cột 2: Trạng thái
            self.table.setCellWidget(row, 3, QLabel("—"))  # Cột 3: Xem
        ...
        self.status_label.setText("Đã xử lý xong một prompt.")
        ...
        if all(self.table.item(r, 2).text().startswith(("✅", "❌", "⏹️")) for r in range(self.table.rowCount())):
            ...
            if self.start_time:
                ...
                self.status_label.setText("Hoàn thành tất cả.")
            self.start_btn.setVisible(True)
            self.cancel_btn.setVisible(False)
            self.is_running = False
    ...
    def retry_failed_prompts(self):
        if not self.failed_prompts:
            QMessageBox.information(
                self, "Thông báo", "Không có prompt lỗi để tạo lại.")
            return

        failed_list = list(self.failed_prompts)
        self._append_log(f"🔁 Đang tạo lại {len(failed_list)} prompt lỗi...")
        ...
        for prompt in failed_list:
            ...
            # Cập nhật trạng thái
            self.table.item(row, 2).setText("🔁 Đang tạo lại...")  # Cột 2: Trạng thái
    ...
    def clear_results(self):
        ...
        self.failed_prompts.clear()
        self.retry_failed_btn.setEnabled(False)
        self._append_log("🧹 Đã xóa kết quả.")
    ...
    def download_video(self, url, prompt):
        ...
        try:
            ...
            self._append_log("❌ Lỗi tải: " + str(e))
    ...
    def merge_videos_with_ffmpeg(self, video_files, output_file):
        ...
        try:
            ...
            self._append_log(f"❌ Lỗi ghép video: {e.stderr}")
    ...
    def download_selected_videos(self):
        ...
        if not selected:
            QMessageBox.information(
                self, "Thông báo", "Vui lòng chọn ít nhất 1 video để tải.")
            return
        ...
        QMessageBox.information(
            self, "Hoàn tất", 
            f"Đã tải xong {success_count}/{len(selected)} video.\n"
            f"Thư mục: {save_dir}")
    ...
    def merge_downloaded_videos(self):
        ...
        if not files or len(files) < 2:
            QMessageBox.information(
                self, "Thông báo", 
                "Vui lòng chọn ít nhất 2 file video để ghép.")
            return
        ...
        if self.merge_videos_with_ffmpeg(files, output_file):
            QMessageBox.information(
                self, "Hoàn tất", 
                f"Đã ghép thành công {len(files)} video thành 1 file:\n{output_file}")
            ...
            QMessageBox.warning(
                self, "Lỗi", 
                "Không thể ghép video. Vui lòng kiểm tra:\n"
